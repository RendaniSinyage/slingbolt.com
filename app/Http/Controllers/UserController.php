...
\Log::info("UserController: Finished calling cloneCompanyDefaults for user: " . $user->id);
    ...
    private function cascadeDeleteCompanyData($companyId)
    {
        ...
        // Tables to exclude from deletion (system/shared tables)
        $excludedTables = [
            'migrations', 'password_resets', 'failed_jobs', 'personal_access_tokens',
            'sessions', 'permissions', 'plans', 'coupons', 'admin_payment_settings',
            'webhook_settings', 'email_template_langs', 'notification_template_langs',
            'ch_favorites', 'ch_messages', 'join_us', 'orders', 'order_products',
            'plan_requests', 'subscriptions', 'user_plans', 'order_coupons',
            'user_coupons', 'transaction_orders'
        ];

        // Tables with special deletion logic (foreign keys, relationships)
        $specialTables = [
            'journal_items' => [
        ...
        $companyIdColumns = [
            ...
            'user_id' => $companyId  // for tables where company is referenced as user_id
        ];
